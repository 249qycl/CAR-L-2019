...
void flexbus_8080_init()
{
    ...
    port_init(PTB19 , ALT5 | HDS);              //RD写使能  PTB19复用为 fb_oe_b  ,配置为 输出高驱动能力 , fb_oe_b 时序符合 8080 总线的 
    port_init(PTD1  , ALT5 | HDS);              //CS片选    PTD1 复用为 fb_cs0_b ,配置为 输出高驱动能力 , fb_cs0_b时序符合 8080 总线的 
    port_init(PTC11 , ALT5 | HDS);              //WR读使能  PTC11复用为 fb_r/w   ,配置为 输出高驱动能力 , fb_r/w  时序符合 8080 总线的 
    ...
    port_init(PTC12 , ALT5 | HDS);              //RS数据/指令选择线  PTC12复用为 fb_ad[27],配置为 输出高驱动能力 , fb_ad[27]作为8080 总线的 
